...
bool CarteSD::ecrire(DonneesTR* dTR) {
	//creation de variable pour les condition si/sinon

	float distanceparcourue = dTR->getDistanceParcourue();
	int consoMaxi = dTR->getConsoMax();
	int consoMoy = dTR->getConsoMoyenne();
	int vitessmoy = dTR->getVitesseMoyenne();
	int vitessmax = dTR->getVitesseMax();
	int regimmax = dTR->getRegimeMax();
	int regimmoy = dTR->getRegimeMoyen();
	int def[4] = { dTR->getDefauts(0), dTR->getDefauts(1), dTR->getDefauts(2),
			dTR->getDefauts(3) };
	int nbdef = dTR->getNbDefaut();
	float lat = dTR->getLatitude();
	float longi = dTR->getLongitude();
	...
	if (fichierSD) {          // si l'ouverture as réussie
		...
		//ecriture des données sur la carte SD, si il n'y a pas de valeur alors on écrit "N"

		fichierSD.print(dTR->getDatation().tm_mday);
		fichierSD.print("/");
		fichierSD.print(dTR->getDatation().tm_mon);
		fichierSD.print("/");
		fichierSD.print(dTR->getDatation().tm_year);
		fichierSD.print(";");
		fichierSD.print(dTR->getDatation().tm_hour);
		fichierSD.print(":");
		fichierSD.print(dTR->getDatation().tm_min);
		fichierSD.print(":");
		fichierSD.print(dTR->getDatation().tm_sec);
		fichierSD.print(";");

		if (distanceparcourue == NULL) {
			fichierSD.print("N");
			fichierSD.print(";");
		} else {
		...
		if (nbdef == NULL) {
			...
			fichierSD.print(dTR->getNbDefaut());
			fichierSD.print(";");
		}
		for (int i = 0; i <= 3; i++) {
			if (def[i] == NULL) {
		...
		if (lat == NULL) {
			...
			fichierSD.print(dTR->getLatitude());
			fichierSD.print(";");
		}
		...
		if (longi == NULL) {
			...
			fichierSD.println(dTR->getLongitude());

		}

		// ecriture des donnée
		// fichierSD.println(/*dTR->getDatation()*/+';'+ String(dTR->getDistanceParcourue())+';' + String(dTR->getConsoMax()) +';'+ String(dTR->getConsoMoyenne()) +';'+  String(dTR->getVitesseMoyenne()) +';'+ String(dTR->getVitesseMax()) +';'+ String(dTR->getRegimeMax()) +';'+ String(dTR->getRegimeMoyen()) +';'+String(dTR->getNbDefaut())+';'+/*String(dTR->getDefauts()) +';'+*/String(dTR->getLatitude()) +';'+ String(dTR->getLongitude()));
...
bool CarteSD::nouveauFichier(String nom) {
	if (SD.exists(nom)) {
		...
		if (fichierSD) {                       // si le fichier as bien été crée
			fichierSD.println(
					"Date;Heure;KM;CMax;CMoy;VMX;VMOY;RMX;RMOY;NBDF;CD1;CD2;CD3;CD4;Long;Lat"); // entête du fichier
			Serial.println(fichierSD.name());                   // debug
